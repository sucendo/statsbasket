	...
	Draggable.prototype.onDragEnd = function( instance, event, pointer ) {
		...
		for( var i = 0, len = this.droppables.length; i < len; ++i ) {
			...
			if( droppableEl.isDroppable( instance.element ) ) {
				...
				//iSucendo
				if (!droppableEl.el.id){
				}else{
					addQuinteto(droppableEl.el.id, 0);
				}
				droppableEl.el.id = instance.element.id;
				droppableEl.el.innerHTML = '<span>'+instance.element.innerHTML+'</span>';
				addQuinteto(instance.element.id, 1);
				//fSucendo
			}
		}
